...
- **GSC/GA4**: `gscService` / `ga4Service` 等で Google 連携を管理。記事評価、改善提案を自動化。`/api/gsc/*`, `/api/ga4/*` と `/api/cron/gsc-evaluate` で定期処理を実行。GSC インポートは 30 日単位で自動分割し、クエリ指標（`gsc_query_metrics`）は 10,000 行を上限として取得。
