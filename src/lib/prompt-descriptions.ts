...
export const PROMPT_DESCRIPTIONS: Record<string, PromptDescription> = {
  'ad_copy_creation': {
    ...
    usage: 'チャット機能で「広告コピーを作成して」と入力された際に使用されます',
    variables: '事業者情報の基本項目（業種、サービス名、ターゲット、エリア）が自動で置換されます'
  },
  'lp_draft_creation': {
    description: 'ランディングページの構成と原稿を作成するプロンプト',
    usage: 'チャット機能で「LPを作成して」「ランディングページ原稿」と入力された際に使用されます',
    variables: '事業者情報に基づいて構造化されたLP原稿を生成します（ヘッダー、問題提起、解決策、特徴、実績、料金、FAQ、CTA）'
  },
  'ad_copy_finishing': {
    description: 'ユーザーから入力された広告コピーを修正・改善するプロンプト',
    usage: 'チャット機能で広告コピーの修正依頼があった際に使用されます',
