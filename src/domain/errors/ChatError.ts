...
export class ChatError extends DomainError {
  ...
  private static getUserMessage(code: ChatErrorCode): string {
    const messages: Record<ChatErrorCode, string> = {
      [ChatErrorCode.NETWORK_ERROR]: 'ネットワーク接続に問題があります。しばらくしてから再度お試しください。',
      ...
      [ChatErrorCode.RATE_LIMIT_EXCEEDED]: '送信回数が制限を超えています。しばらくしてから再度お試しください。',
      [ChatErrorCode.SESSION_NOT_FOUND]: 'チャットセッションが見つかりません。',
      [ChatErrorCode.SESSION_EXPIRED]: 'チャットセッションが期限切れです。新しいチャットを開始してください。',
      ...
      [ChatErrorCode.AI_SERVICE_ERROR]: 'AIサービスに問題が発生しています。しばらくしてから再度お試しください。',
      [ChatErrorCode.MODEL_NOT_AVAILABLE]: '選択されたAIモデルが利用できません。他のモデルをお試しください。',
      [ChatErrorCode.UNKNOWN_ERROR]: '予期せぬエラーが発生しました。サポートにお問い合わせください。',
    };
