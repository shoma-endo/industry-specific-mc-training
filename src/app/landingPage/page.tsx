...
type CTAForm = { heading: string; description: string; fields: string[]; submitLabel: string };
...
export default async function LandingPage() {
  ...
  if (!data) return <p>データ取得時にエラーが発生しました。</p>;
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {data.features?.map((feature, i) => (
            <div key={i} className="border p-4 rounded shadow-sm">
              ...
              <div className="mt-2 text-sm text-gray-500">icon: {feature.icon}</div>
            </div>
          ))}
        </div>
      </section>
