...
// クライアントコンポーネントとして分離せず、シンプルにサーバーコンポーネントで完結させる
function GoogleAdsSetupContent({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/setup" className="text-sm text-gray-500 hover:text-gray-900 flex items-center gap-1">
        ...
        <CardContent className="space-y-4">
          <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-800 border border-blue-100">
            ...
            <ul className="list-disc pl-5 space-y-1">
              <li>広告キャンペーン、クリック数、コンバージョン数などのパフォーマンスデータを取得・分析します。</li>
              <li>取得したデータは、AIによる広告改善提案の生成のみに使用されます。</li>
              <li>本サービスがGoogle Adsの設定を自動で変更したり、広告を出稿することはありません。</li>
            </ul>
          </div>

          {!success ? (
            <Button asChild className="w-full bg-blue-600 hover:bg-blue-700">
              <Link href="/api/google-ads/oauth/start">
                Google Ads と連携する
              </Link>
            </Button>
          ) : (
            <div className="space-y-4">
              ...
              <p className="text-xs text-center text-gray-500">
                ※ 連携を解除する場合は、Google アカウントの「アカウントにアクセスできるアプリ」から削除してください。
              </p>
            </div>
          )}
        </CardContent>
