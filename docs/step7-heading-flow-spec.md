...
5. 保存のたびに完成形を自動再結合する。
6. **最後の見出し保存後、Step6 書き出し案＋完成形を Canvas で再表示し最終確認する。**
...
| 項目 | 仕様 |
|------|------|
| ボタン配置 | Step7 の Canvas 操作エリア（StepActionBar または CanvasPanel 内）。`戻る` ボタンの近くなど、目立ちすぎない位置。 |
| 表示条件 | Step7 表示中 かつ `session_heading_sections` が1件以上存在する場合のみ表示 |
...
#### heading_key 衝突時の代替案（実装時選択）

上記の「衝突時はエラー返却・Step5 見直しを促す」はデータ整合性を優先する方針であるが、万が一の衝突時にユーザーが Step5 に戻って修正する必要がある点は UX 上のハードルとなりうる。以下の代替案を検討し、実装時に選択する。

| 案 | 内容 | メリット | デメリット |
|----|------|----------|------------|
| **A. 現状維持** | 衝突時はエラー表示のまま。ユーザーに Step5 見直しを促す。 | 実装がシンプル。データ整合性が明確。 | 衝突時の UX 負荷が高い。 |
...
| **C. 段階的アプローチ**（推奨） | 現状維持でリリースし、実運用で衝突発生をモニタリング。実際に発生した場合に B 案などを検討。 | 初期実装が軽い。衝突が稀である前提では十分。 | 衝突発生時に改修が必要。 |
...
**推奨**: 初回リリースは **C. 段階的アプローチ** とする。SHA-256 8文字では衝突は極めてまれなため、まず現状維持で運用し、衝突が実際に報告された場合に **D 案（retry_count 付与）** または B 案を導入する。衝突を事前に完全に避けたい場合は **E 案（UUID v4）** を採用する。
...
| 項目 | 仕様 |
|------|------|
| 最大リトライ回数 | 3回（初回含め計4回実行） |
| リトライ間隔 | 指数バックオフ: 200ms → 400ms → 800ms。各リトライ前に待機する。 |
