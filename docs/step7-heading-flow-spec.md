...
| 項目 | 仕様 |
|------|------|
| ボタン配置 | Step7 の Canvas 操作エリア（StepActionBar または CanvasPanel 内）を基本とする。旧step6データからの移行導線として、step6 表示中にも同位置相当で表示可。`戻る` ボタンの近くなど、目立ちすぎない位置。 |
| 表示条件 | Step7 表示中は表示可。加えて、旧step6データを step7 へ移行する導線として step6 表示中にも表示可。 |
